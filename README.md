...
- `RHLean.XiCoefficients`: defines the shared `Xi` coefficient sequence interface, including the analytic certificate needed by the Jensen approximants.
- `RHLean.Jensen.Polynomial`: defines the minimal Jensen polynomial route, including its existential coefficient boundary and the theorem from Jensen hyperbolicity to Laguerre-Polya membership for `Xi`.
